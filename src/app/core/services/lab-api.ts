  ...
  export class LabApiService {
    ...
    getAdminWalletBalance(labId: number, page: number = 0, size: number = 10): Observable<any> {
  const params = new HttpParams()
    .set('page', page.toString())
    .set('size', size.toString());

  return this.http.get(
    `${this.BASE_URL}/api/v1/wallet/summary/${labId}/0`,
    { params }
  );
}
...
getLabWallet(labId: any): Observable<any> {
  return this.http.get(`${this.BASE_URL}/api/v1/lab-wallet/${labId}`);
}

  getLabWalletTransactions(startDate: string, endDate: string): Observable<any> {
    const params = new HttpParams()
      .set('startDate', startDate)
      .set('endDate', endDate);

    return this.http.get(
      `${this.BASE_URL}/api/v1/lab-wallet/get-lab-credit-transactions`,
      { params }
    );
  }
